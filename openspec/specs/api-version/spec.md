...
The system SHALL expose a `GET /api/version` endpoint that returns a JSON object containing the current application version read from `package.json`.

#### Scenario: Successful version query
- **WHEN** a client sends a `GET` request to `/api/version`
